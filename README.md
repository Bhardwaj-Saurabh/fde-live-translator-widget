...
| **Node gateway** | ✅ Implemented — logging middleware, proxy, request-ID tracing | `backend/gateway-node/` |
| **Python AI service** | ✅ Implemented — style-encoded LLM prompt, two-tier cache, single-flight | `backend/ai-service-python/` |
...
- [x] **LLM** — translate EN → **Hindi** (natural hi-IN in Devanagari script, not romanized Hinglish) via a real LLM call.
- [x] **Caching** — two-tier (in-memory + SQLite), keyed by a hash of `(text, target)`; identical input never calls the LLM twice; `cached`/`latencyMs`/`/stats` reflect it.
- [x] **Logging** — one structured line per request in the gateway **and** per translation in the AI service; greppable.
- [x] **Tracing** — a request ID at the gateway (reusing an inbound `X-Request-Id` if present, else generated), forwarded to the AI service, and logged by both; one request is greppable end-to-end across both services by that single ID.
- [x] **Performance** — meets every SLA in `benchmark/sla.json`; `python benchmark/bench.py` exits `0`.
- [x] **Runs locally** — each service starts with a single documented command; secrets come from `.env` (never committed).
- [x] **Deployed on Fly.io** — both services deployed; gateway public at `https://saurabh-livetranslate-gw.fly.dev`, AI service private (flycast only).
- [x] **Contract** — every endpoint matches the shapes above; the provided widget works unmodified.
...
- [x] `POST /translate` → `{ "translated": string, "cached": boolean, "latencyMs": number, "model": string }`
- [x] `POST /translate/batch` → `{ "results": [{ "translated": string, "cached": boolean }], "latencyMs": number }`
- [x] `GET /health` → `{ "status": "ok", ... }` and `GET /stats` → cache stats incl. a hit rate
- [x] Status codes: `400` invalid input, `501` not implemented, `502` upstream failure
- [x] The provided widget works **unmodified** against the gateway at `:8787`

**LLM**
- [x] Output is natural **Hindi (hi-IN)**, translation only — no preamble, no wrapping quotes
- [x] Numbers, prices (`$`), and product/model codes are preserved
- [x] Provider is swappable via env; the API key is read from `.env`, never hard-coded
- [x] **Errors surface, never swallowed** — on a provider/LLM failure the request returns `502` and the error is logged; the service **never returns the untranslated input as if it succeeded** (a silent "return the original text" fallback is an automatic fail)

**Caching (hard)**
- [x] Identical `(text, target)` **never** calls the LLM twice
- [x] `cached: true` appears **only** when the response came from cache
- [x] Two tiers — in-memory **and** SQLite; the SQLite cache **survives a restart**
- [x] Cache key is a hash of `(text, target)`

**Logging & tracing**
- [x] Gateway logs one structured line per request: method, url, status, duration (ms)
- [x] AI service logs one structured line per translation: cached, latencyMs, chars
- [x] A request ID is set at the gateway (inbound `X-Request-Id` reused if present, else generated), forwarded to the AI service, and logged by both — one request is greppable end-to-end across both services by that single ID

**Performance (SLA gate)**
- [x] `python benchmark/bench.py` exits `0` — every SLA in `benchmark/sla.json` passes

**Deploy**
- [x] Each service starts locally with a single documented command
- [x] Both services are deployed to Fly.io and the extension works against the public gateway URL

**Hygiene**
- [x] `.env`, `node_modules/`, `.venv/`, `*.db`, `*.log` are git-ignored and NOT committed
...
- **Translation style:** the prompt in `backend/ai-service-python/lib/llm.py` is distilled from
  ...
  Hindi UI strings, and authority guides (Mozilla l10n, CLDR, W3C). If you change the prompt,
  delete `translations.db` — the cache key has no prompt version.
- **Python env:** one [uv](https://docs.astral.sh/uv/) venv at the assignment root —
  `uv venv && uv pip install -r backend/ai-service-python/requirements.txt -r backend/ai-service-python/requirements-dev.txt`
- **Run the services:**
  ```bash
